...
module MicsHelper
  ...
  def mic_order_any?(mic, num)
    Mic.select { |m| m.date == mic.date && m.period_id == mic.period_id && m.order == num }.any?
  end

  # 該当分割マイク練でn番目に入ってるバンドの名前を取得
  def ordered_band_name(mic, num)
    Mic.select { |m| m.date == mic.date && m.period_id == mic.period_id && m.order == num }.first.band.name
  end
